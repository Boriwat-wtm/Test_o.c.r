...
class TestTruthDefects:
    """ตรวจว่า "เฉลย" เองเสียไหม — คนละเรื่องกับตรวจว่า OCR อ่านผิดไหม

    ทุกตัวเลขในโปรเจกต์เชื่อถือได้เท่ากับเฉลยเท่านั้น ที่ผ่านมาเจอเฉลยเสีย
    สองรอบและทั้งคู่เจอโดยบังเอิญ กฎชุดนี้จึงถามล่วงหน้าแทนที่จะรอให้สงสัยเอง
    """

    def test_จับวรรณยุกต์ที่หลุดจากพยัญชนะ(self):
        """เคสจริงที่เจอในเฉลย — ้ ลอยอยู่ท้ายบรรทัดโดยไม่มีพยัญชนะเกาะ"""
        got = truth_defects(["ในที่เปิดเผย  ณ  สำนักงานที่ดิน ้"])
        assert len(got) == 1
        assert got[0][1] == "สระ/วรรณยุกต์หลุดจากพยัญชนะ"

    def test_จับบรรทัดที่ขึ้นต้นด้วยวรรณยุกต์(self):
        """เคสจริง — ทั้งบรรทัดมีแต่ ้ ตัวเดียว"""
        got = truth_defects(["้"])
        assert got and got[0][1] == "บรรทัดขึ้นต้นด้วยสระ/วรรณยุกต์"

    def test_จับวรรณยุกต์ซ้อนกันเอง(self):
        assert truth_defects(["ที่่ดิน"])

    def test_เงียบเมื่อเฉลยปกติ(self):
        """สำคัญพอ ๆ กับการจับได้ — เตือนผิดบ่อยแล้วคนจะเลิกอ่าน"""
        clean = [
            "มาตรา ๙  ภายใต้บังคับกฎหมายว่าด้วยการเหมืองแร่",
            "อาศัยอำนาจตามความในมาตรา  ๖  แห่งพระราชบัญญัติ",
            "ให้ไว้ ณ วันที่ ๓๐ พฤศจิกายน พ.ศ. ๒๔๙๗",
        ]
        assert truth_defects(clean) == []

    def test_บอกเลขบรรทัดถูกต้อง(self):
        """ต้องชี้ได้ว่าอยู่บรรทัดไหน ไม่งั้นคนหาไม่เจอ"""
        got = truth_defects(["ปกติ", "ปกติ", "้"])
        assert got[0][0] == 2
