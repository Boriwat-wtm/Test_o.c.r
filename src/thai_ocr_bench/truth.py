...
#
# คนละอาการกับที่ repair_sara() ซ่อม — ตัวนั้นคือ า ถูกแมปเป็น ำ ทั้งไฟล์
# ส่วนตัวนี้คือ ำ ถูกแยกเป็นสองชิ้นโดยมีช่องว่างคั่น
#     'อ านาจ'  ->  'อำนาจ'      'ท าประโยชน์'  ->  'ทำประโยชน์'
#     'ก าหนด'  ->  'กำหนด'      'ส านักงาน'    ->  'สำนักงาน'
#
# ปลอดภัยด้วยเหตุผลเดียวกับ repair_sara: ภาษาไทยไม่มีคำที่ขึ้นต้นด้วยสระอา
# ช่องว่างหน้า 'า' จึงไม่มีทางถูกต้องอยู่แล้ว
_SPLIT_AM = re.compile(f"([ก-ฮ]) {_SARA_AA}")


def join_split_sara_am(text: str) -> str:
    """ต่อสระอำที่ถูกหั่นกลับเป็นตัวเดียว

    ถ้าไม่ซ่อม ตัววัดจะตัดช่องว่างทิ้งก่อนเทียบ ทำให้ 'ท า' กลายเป็น 'ทา'
    แล้วชนกับ 'ทำ' ที่ OCR อ่านถูก — กลายเป็นลงโทษ engine เพราะเฉลยเสียเอง

    วัดจริงกับกฎกระทรวง ๕๙: มี 113 จุดใน 5 หน้า และเป็นเหตุให้ทุก engine
    ทำคะแนนแย่ลงพร้อมกันราว 1.2 จุด CER ในเอกสารฉบับนั้นฉบับเดียว
    ซึ่งเคยถูกเข้าใจผิดว่าเป็นเพราะเอกสารยากกว่า
    """
    return _SPLIT_AM.sub(rf"\1{_SARA_AM}", text)


def _clean_lines(raw: str) -> list[str]:
    if needs_sara_repair(raw):
        raw = repair_sara(raw)
    raw = join_split_sara_am(raw)
...
# สระกับวรรณยุกต์ต้องเกาะพยัญชนะเสมอ ลอยเดี่ยวหรือมีช่องว่างคั่นไม่ได้
_COMBINING = "ะาำิีึืุูํ็่้๊๋์"

# ตรวจเฉลย ไม่ใช่ตรวจผล OCR — ทุกกฎต้องตัดสินได้โดยไม่ต้องรู้ว่าเอกสารเขียนว่าอะไร
#
# ทำไมต้องมี: ทุกตัวเลขในโปรเจกต์นี้เชื่อถือได้เท่ากับเฉลยเท่านั้น ถ้าเฉลยผิด
# โดยไม่มีใครรู้ ทุกข้อสรุปผิดตามหมด และที่ผ่านมาเจอเฉลยเสียสองรอบแล้วทั้งคู่
# โดยบังเอิญ — ฟอนต์แมปผิด (แก้ด้วย broken_glyph_rate) กับสระอำถูกหั่น
# (แก้ด้วย join_split_sara_am) ทั้งสองรอบใช้เวลานานกว่าจะสงสัย
#
# ตัวนี้จึงถามล่วงหน้าว่า "เฉลยหน้านี้ดูผิดปกติไหม" แทนที่จะรอให้สงสัยเอง
_TRUTH_CHECKS: tuple[tuple[str, re.Pattern[str]], ...] = (
    ("สระ/วรรณยุกต์หลุดจากพยัญชนะ", re.compile(f"[ก-ฮ] [{_COMBINING}]")),
    ("บรรทัดขึ้นต้นด้วยสระ/วรรณยุกต์", re.compile(f"^[{_COMBINING}]")),
    ("วรรณยุกต์ซ้อนกันเอง", re.compile(r"[่้๊๋]{2,}")),
    ("อักขระแปลกแทรกกลางคำไทย", re.compile(r"[ก-๛][=<>*@^~`|][ก-๛]")),
)


def truth_defects(lines: list[str]) -> list[tuple[int, str, str]]:
    """หาจุดที่เฉลยเองน่าจะเสีย คืน (ดัชนีบรรทัด, อาการ, ข้อความรอบ ๆ)

    ตั้งใจให้ "รายงาน" ไม่ใช่ "ซ่อมเอง" — อาการที่ยังไม่เคยเจอมาก่อน
    เราไม่รู้ว่าต้นฉบับควรเป็นอะไร เดาแล้วซ่อมคือสร้างเฉลยผิดชนิดใหม่
    ให้คนดูแล้วตัดสิน แบบเดียวกับที่ suspect.py ทำกับผล OCR

    วัดกับเฉลย 66 หน้าตอนเพิ่มกฎนี้: เจอ 2 จุด เตือนผิด 0 จุด
    ทั้งสองเป็นวรรณยุกต์ ้ ที่หลุดจากพยัญชนะจริง
    """
    out: list[tuple[int, str, str]] = []
    for i, line in enumerate(lines):
        for label, pattern in _TRUTH_CHECKS:
            for m in pattern.finditer(line):
                out.append((i, label, line[max(0, m.start() - 20) : m.end() + 20]))
    return out
